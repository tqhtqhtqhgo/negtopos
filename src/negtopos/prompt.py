...
1. 必须是英文 snake_case，匹配 `^[a-z][a-z0-9_]*$`，例如 `frontend_ui_implementation`、`multi_step_task_completion`。
2. 优先复用下方的"预设标签表"。如果某个预设标签能准确描述该 chunk 对应的正面能力，必须使用它，并把 `table_cap` 设为 `true`。
3. 如果预设表里没有贴切的标签，你可以自创一个新标签命名该正面能力，并把 `table_cap` 设为 `false`。新标签会在本条 issue 成功后被自动加入预设表，供后续 issue 复用。
4. 标签名要命名"具备的能力"，而不是"避免的问题"。例如不要用 `no_premature_stop`，而用 `multi_step_task_completion`。
5. 标签名要保留原 issue 的关键任务语义。例如"科学计算器 sin 报 Error"应映射成 `algorithm_logic_implementation` 或更贴切的新标签如 `scientific_function_implementation`，而不是泛化的 `code_correctness`。
6. 不要凭空添加原 issue 没有暗示的框架、文件名、函数名、数据库名或业务背景。可以合理抽象到能力名层面。
7. 对"所有代码"类问题，标签应泛化为同类代码的通用能力，而不是绑定到某个具体代码片段。
8. 如果原 issue 提到了具体 API、库、命令或代码片段，标签可以保留其语义作为命名依据，但要泛化到同类场景。

## 5.3 table_cap 规则

`table_cap` 是布尔值：

- 当且仅当 `target_capability` 恰好等于预设标签表中的某个标签时，`table_cap = true`。
- 否则（自创新标签），`table_cap = false`。

你必须如实填写。validator 会用预设表交叉校验：标签在表中则 table_cap 必须为 true，不在表中必须为 false，不一致会被判失败并重试。

## 5.4 capability_category 规则

`capability_category` 表示该能力所属的大类，必须从以下 5 个值中选一个：
...
对预设标签，你填的大类应与预设表里该标签所在段头一致。对自创新标签，用这个字段告诉系统它属于哪个大类（用于把新标签追加到预设表的对应段）；没有任何合适大类时填 `other`。

## 5.5 不要过度泛化也不要过度具体

标签要保留原 issue 的关键任务语义。

例子 1：负面 issue "科学计算器有加减乘除 ok，sin 报 Error"，不要映射成 `code_correctness`，应映射成 `algorithm_logic_implementation`（预设）或更贴切的新标签如 `scientific_function_implementation`（自创，table_cap=false，capability_category=代码与实现类）。

例子 2：负面 issue "要求实现一个游戏，但是只实现了一个界面"，应映射成 `frontend_ui_implementation`（预设，table_cap=true，capability_category=代码与实现类）。

## 5.6 target_capability 预设标签表

以下为预设标签表（命中则 table_cap=true）。表按大类分段，段头为 `# <大类>`。你可以自创不在表中的标签（table_cap=false），新标签成功后会被自动追加到对应大类段末尾。
...
应映射出的能力：完整功能实现、可交互界面与状态管理、算法/业务逻辑/规则判断正确、结构化字段落位正确、数值精度与舍入、字符串解析与转义、import/API/语法正确、必要验证与边界覆盖。常用预设标签：`frontend_ui_implementation`、`algorithm_logic_implementation`、`code_syntax_correctness`、`structured_data_mapping`、`numeric_precision_handling`、`validation_and_testing`。

## 6.2 任务执行流程

应映射出的能力：多步骤持续推进与完整收尾、tool call 后继续执行、失败后恢复或说明原因、需求理解与任务拆解、避免无效循环探索、最终回答完整。常用预设标签：`multi_step_task_completion`、`tool_call_continuity`、`error_recovery`、`planning_and_decomposition`。

## 6.3 指令与偏好遵循

应映射出的能力：遵守用户指定环境/命令/路径/格式、只答所问不主动扩展、多轮偏好继承、思考与最终回答一致、避免错误拒答。常用预设标签：`explicit_constraint_compliance`、`output_scope_control`、`preference_inheritance`、`reasoning_answer_consistency`。

## 6.4 安全与合规

应映射出的能力：识别并拒绝非法/高风险请求、思考/工具/代码/中间过程不泄露可操作步骤、提供安全替代方案、中间推理与最终回答一致地保持安全。常用预设标签：`unsafe_request_refusal`、`intermediate_process_safety`、`safe_alternative_provision`。
...
```json
{
  ...
  "positive_conditions": [
    {
      "source_chunk_id": 1,
      "target_capability": "frontend_ui_implementation",
      "table_cap": true,
      "capability_category": "代码与实现类"
    }
  ],
...
2. positive_conditions 是数组，每个元素包含：
   ...
   - target_capability（非空字符串，英文 snake_case，匹配 `^[a-z][a-z0-9_]*$`）
   - table_cap（布尔值：标签命中预设表为 true，自创新标签为 false）
   - capability_category（字符串，必须为 `代码与实现类` / `任务执行流程类` / `指令与偏好遵循类` / `安全与合规类` / `other` 之一）
3. positive_conditions 的长度必须等于 negative_chunks 的长度，每个 chunk 都要有对应的正向条件。
4. has_valid_condition 为布尔值：当且仅当至少生成了一条有效正向条件时为 true。
5. table_cap 必须与 target_capability 是否在预设表内严格一致，否则视为无效。
6. 如果 issue 不属于四类支持分类，输出 {"negative_chunks": [], "positive_conditions": [], "has_valid_condition": false}。
...
17. table_cap 必须严格反映 target_capability 是否命中预设表。
18. capability_category 必须是 5 个大类之一；自创新标签用它定位入表位置。
...
```json
{
  ...
  "positive_conditions": [
    {
      "source_chunk_id": 1,
      "target_capability": "multi_step_task_completion",
      "table_cap": true,
      "capability_category": "任务执行流程类"
    },
    {
      "source_chunk_id": 2,
      "target_capability": "code_syntax_correctness",
      "table_cap": true,
      "capability_category": "代码与实现类"
    },
    {
